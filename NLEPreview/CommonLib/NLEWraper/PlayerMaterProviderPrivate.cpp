...
NLEComPtr<INLETimelineBase> PlayerMaterProviderPrivate::CreateTimelineForScreenRecording(const QString &path)
{
	...
	if (rootTimeline && childCount > 0)
	{
		...
		if (webCamTimeline && (desktopCamTimeline || gameCamTimeline))
		{
			...
			int width = desktopSize.Width * 0.4;
			int height = width *  desktopSize.Height / desktopSize.Width;
			rectF.Height = 1.0f * height / desktopSize.Height;
...
{
	return SUCCEEDED(mediaTimeline->GetRender(ppAudioRender,ppVideoRender));
}
