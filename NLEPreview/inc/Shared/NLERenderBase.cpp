...
HRESULT __stdcall CNLERenderBase::Prepare(INLEFrame* pFrame)
{
	...
	NLERange rngNumber = pProp[kRender_RangFrameNumber];
...
{
	return GetCurrentSession() != m_oPrevSession;
}
...
HRESULT CNLERenderBase::RefreshEventPoint(NLEFrameIndex refresh_pos)
{
	...
	NLERange rdrRange = pProp[kRender_RangFrameNumber];
...
HRESULT CNLERenderBase::UpdateRenderList(int nFrameOffset, bool &notEndOfTimeline, bool seek_op)
{
	...
	*/
	...
	NLERange rngRender = (CNLEValue)pProp[kRender_RangFrameNumber];
...
HRESULT CNLERenderBase::MyPostCompInit()
{
	...
	if (NULL == m_pPostRequest)
	{
		...
		{
			LOGGER_ERROR_(L"Failed to create instance postrequest"); _ASSERT(FALSE);
			return hr;
		}
	}
...
{
	NLEComPtr<INLETimelineBase> pTimline = GetTimeline(); _ASSERT(pTimline);
...
HRESULT CNLERenderBase::DoPostProcess(INLEFramePtr pCurFrame, INLEFramePtr pGraphics, INT nFlag /*= 0*/)
{
	...
	if (m_pPostProcess)
	{
		_ASSERT(m_pPostRequest->GetCurrentFrame() == pCurFrame);
		_ASSERT(m_pPostRequest->GetGraphics() == pGraphics);
