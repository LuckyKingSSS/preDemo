...
private:
	void SetRenderDirty(bool isDirty);

protected:
	typedef NLEComPtr<INLETimelineBase> INLETimelineBasePtr;
	struct TimelineCompare
	{
		/**
		 *@brief 偏序函数，pThis < pThat 返回true，否则返回false
		 */
		bool operator()(INLETimelineBasePtr pThis, INLETimelineBasePtr pThat) const;
	};	
	typedef std::set<INLETimelineBasePtr, TimelineCompare> TIMELINE_LIST;
