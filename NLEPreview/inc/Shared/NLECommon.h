...
namespace NLECommon
{
	...
	NLECOMMON_API NLEComPtr<IThreadPool> GetThreadPool();
	 ...
	 __inline std::wstring GetAudioFilterLibFullPath(LPCWSTR pszFilterID)
	 {
		 ...
		 INT nFilterCount = 0;
		 pAudioResourceInfo->GetInt(NLEKey::Filter::kFilterCount, &nFilterCount); CHECK(hr); _ASSERT(nFilterCount > 0);
...
#define MSec2Sec(ms)  NLECommon::Time::MSecToSec((ms))
		...
		// unit: Microsecond
		static DWORD NLEGetMicrosecond()
		{
			LARGE_INTEGER liCurrent = { 0 };
			QueryPerformanceCounter(&liCurrent);
			return (DWORD)(liCurrent.QuadPart * 1000000 / GetSecondCount());
		}

#else
		#define Sleep(a) usleep((a) * 1000)
		DWORD NLEGetMicrosecond();		
#endif
		/**@brief 微秒(us)为单位*/
	...
	__inline void VideoFrameCopy(NLEComPtr<INLEFrame> pDst, NLEComPtr<INLEFrame> pSrc)
	{
        ...
        _ASSERT(dstMedia.video.nWidth == srcMedia.video.nWidth);
		_ASSERT(dstMedia.video.nHeight == srcMedia.video.nHeight);
		_ASSERT(pSrc->GetDataSize() == pDst->GetDataSize());
...
//            strFileName.find_last_of(L'.');
			...
			strExt = pszExt.c_str();
			strExt += L";";
			std::wstring strAllImageFormat = L"*.bmp;*.jpg;*.jpeg;*.jp2;*.gif;*.png;*.ico;*.html;*.info;*.miff;*.m2v;*.mpc;*.ico;"
	...
	__inline int CalcAuidoFrameSize(NLERational framerate, int nBytesPerSec, int index)
	{
		...
		_ASSERT(abs(dTheorySize - nCount * 1.0) < 4.0);
#endif
		return nCount;
	}
