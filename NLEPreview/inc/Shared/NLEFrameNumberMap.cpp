...
NLEFrameIndex NLEFrameNumberMap::GetSourceFrameIndex(const NLEFrameIndex frmIndexRender)
{
	...
	_ASSERT(indexMap.size() == m_ralFromTo.den);
...
{
	_ASSERT(newRalFromTo.den != 0 && newRalFromTo.num != 0);
    ...
    _ASSERT(frmCountSrc == newRalFromTo.num);
    _ASSERT(frmCountRdr == newRalFromTo.den);
	...
	_ASSERT((INT)oMap.size() == frmCountRdr);
	...
	else
	{
		...
		for (int i = 0; i <= (frmCountRdr - 1); i++)
		{
			oMap[i] = (NLEFrameIndex)(dInterval * i + 0.5);
		}
	}
	_ASSERT(m_arrFrameIndiesMap[0] == 0);
	_ASSERT(m_arrFrameIndiesMap[frmCountRdr - 1] <= frmCountSrc - 1);
