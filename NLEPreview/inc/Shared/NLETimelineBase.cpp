...
// 	 if (pName)
// 	 {
		 _ASSERT(name.length() < NAME_LEN_MAX);
		 wcscpy_s(m_pszName, NAME_LEN_MAX, name.c_str());
// 	 }
	 return S_OK;
 }
 ...
 HRESULT CNLETimelineBase::OnSetLevel(LPCWSTR keyName, CNLEValue& nleLevel)
 {
	 ...
	 hr = pParent->RemoveClip(this); CHECK(hr);
	 m_pProperties[kLevel] = nleLevel;
	 hr = pParent->AddClip(this);
	 return hr;
 }
...
HRESULT CNLETimelineBase::Init()
{
	...
	NLERange defaultRenderRange(0, 0);
	m_pProperties[kSource_RangeFrameNumber] = defaultRenderRange; // NLECommon::Const::RANGE_INFINITY;
	m_pProperties[kRender_RangFrameNumber] = defaultRenderRange; // NLECommon::Const::RANGE_INFINITY;
	...
	int totalFrameCount = NLECommon::Const::RANGE_INFINITY.Count();
	...
	m_pProperties[kRender_RotationAngle] = nRotationAngle;
...
HRESULT __stdcall CNLETimelineBase::RemoveClip(INLETimelineBase* pClip)
{
	...
	TIMELINE_LIST::iterator itClip = m_lstTimeline.find(pClip);
...
HRESULT CNLETimelineBase::OnSetRenderPostion(LPCWSTR keyName, CNLEValue& nleValue)
{
	...
	pParent->RemoveClip(this);
	m_pProperties[kRender_Position] = nPos;
	pParent->AddClip(this);
...
{	
	...
	if (m_lstTimeline.find(pClip) != m_lstTimeline.end())
	{
		return E_NLE_ALLREADY_EXISTS;
	}

	m_lstTimeline.insert(pClip);
...
/**
 *@brief  	LessThan判断函数,p1是否小于p2?
 *@param	p1 第一操作数
 *@param	p2 第二操作数
 *@return 	bool 结果值，小于返回true,不小于返回false
 *@note		1.必须是严格的弱序关系!!
 */
bool CNLETimelineBase::TimelineCompare::operator()(INLETimelineBasePtr p1, INLETimelineBasePtr p2) const
{	
