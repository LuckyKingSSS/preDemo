...
public:
	...
	*@return
	*/
	virtual void __stdcall SetTimeInfo(float startTime, float duration) = 0;
	/**
	*@brief     返回对象时间信息
	*@param  startTime起始时间点，  duration  持续时长
	*@return
	...
	*@return bool
	*/
	virtual void __stdcall GetMouseInAreaType(int x, int y, NLEMouseType* mt) = 0;
	/**
	*@brief     绘制对象
	*@param pFrame 背景图
	...
	*@return
	*/
	virtual void __stdcall SetDragParam(int x, int y, int cx, int cy, int nAngle) = 0;
	/**
	*@brief      设置分组信息
	*@param nGroup 分组编号
	*@return
