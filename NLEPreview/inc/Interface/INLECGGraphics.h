...
public:
	...
	virtual void  __stdcall SetBorderWidth(float fWidth) = 0;
...
public:
	...
	virtual void __stdcall GetOrigSceneWH(int& iW, int& iH) = 0;
	virtual void __stdcall SetDstSceneWH(int iW, int iH) = 0;

	//形状位置相关函数
	virtual void  __stdcall SetRotate(float fRotate) = 0;
	//获取图形相对于原始外部坐标的偏移量，由旋转，边框厚度引起的。
	virtual void  __stdcall GetOffSet(int& iOffSetX, int& iOffSetY) = 0;
