...
#define SET_TYPE_X(v, t, type)			t = L ## #type; \
	type& x = (type&)v
		...
		if (RTTI_TYPE_DEAL(v, CNLEValue)){
			...
			swprintf(tmpStr, 128, L"t:%ls, v:%ls", x.Type().c_str(), x.ToString().c_str());
			str = tmpStr;
		}
		else {
			NLE_ASSERT(0);
...
#ifdef WIN32
			swscanf_s(str.c_str(), L"t:%l[^,], v:%l[^\0]", typeStr, MAX_PATH, valStr, MAX_PATH);
#else
...
#define SET_TYPE_X(type) type x
		...
		if (RTTI_TYPE_DEAL(t, CNLEValue)) {
			...
			WCHAR pTempBuf[MAX_PATH] = { 0 };
#ifdef WIN32
			wcscpy_s(pTempBuf, str.c_str());

			swscanf_s(pTempBuf, L"t:%l[^,], v:%l[^\0]", typeStr, MAX_PATH, valStr, MAX_PATH);
#else 
            wcscpy_s(pTempBuf, NULL,str.c_str());	

			swscanf_s(pTempBuf, L"t:%l[^,], v:%l[^\0]", typeStr, valStr);
#endif
			
			
			return String2Value(typeStr, valStr, v);
