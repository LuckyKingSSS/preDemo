...
typedef struct NLEBlob
{
	...
	NLEBlob(LPBYTE pData, UINT uSize, BOOL bCopy = FALSE, LPVOID pExtend = NULL)
	{
		if (bCopy && uSize > 0)
		{
			this->pData = new BYTE[uSize];
			if (this->pData)
			{
				memcpy(this->pData, pData, uSize);
			}
		}
		else
		{
		...
		this->uSize = uSize; this->bCopy = bCopy; this->pExtend = pExtend;
	}

	NLEBlob& operator =(const NLEBlob& oBlob)
	{
		LPBYTE pData = oBlob.pData;
		UINT uSize = oBlob.uSize;
		BOOL bCopy = oBlob.bCopy;
		LPVOID pExtend = oBlob.pExtend;
	...
	void Clear()
	{
		...
		this->uSize = 0; this->bCopy = FALSE; this->pData = NULL; this->pExtend = NULL;
	}
	UINT	uSize;
	LPBYTE	pData;
